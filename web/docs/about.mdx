...
Supabaseは、Firebaseに代わるオープンソースのソフトウェアです。プロダクトを作るために必要なバックエンド・サービスをすべて提供します。
すべてを利用することも、必要なサービスだけを利用できます。

ホスティングされたプラットフォームで、[プロジェクトをはじめる](https://app.supabase.com)、もしくはSupabaseを自分で、[Supabaseをホスト](/docs/guides/hosting/overview)しまう。
