...
>>>>>>> 4bf4b0b8a3915cc3fa51a2f8d3d4c5eece55f5f3
:::
<!-- textlint-enable ja-technical-writing/ja-no-mixed-period -->
...
これらの変数は、データベースで[行レベルのセキュリティ](/docs/guides/auth#row-level-security)を有効にしているため、WebアプリがパブリックなanonキーでSupabaseクライアントを初期化するために使用されます。
...
logIn, logOut, currentUser and access the the `supabase` authenticate client. We'll use it to get an instance
