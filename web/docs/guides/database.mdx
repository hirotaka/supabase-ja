...
Supabaseは、非常にスケーラブルなリレーショナルデータベースである[Postgres](/docs/postgres/server/about)の上に構築されています。
...
- Supabaseは、弊社の[リアルタイム・サーバー](https://github.com/supabase/realtime)を使用して、Postgresをリアルタイム機能で拡張しています。
...
例えば、次のように個別のテーブルにのみリアルタイム性を持たせることができます。
...
| セキュリティー上の注意 |
| - |
| リアルタイム・セキュリティーの現在の状態については、[プライベートなテーブルのリアルタイムを無効化](https://supabase.io/docs/guides/auth#disable-realtime-for-private-tables)をご確認ください。 |
...
エラー表示される場合があります。リストアでは、テーブルが存在する場合はドロップを試み、存在しない場合はエラーが表示されます。これらのエラーは情報提供のみであり、心配する必要はありません。
