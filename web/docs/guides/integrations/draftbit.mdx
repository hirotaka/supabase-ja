...
- **Project URL**セクションで、URLを選択しコピーします。これは、Supabase REST APIのBase URLです。Draftbitアプリに接続する際必要となります。
- また、`Project API keys`で、`anon`の下にあるAPIキーを選択し、コピーします。これはSupabaseデータベースへのすべてのリクエストに必要です。
