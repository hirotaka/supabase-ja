...
>>>>>>> 29bb77e2b0f001ab899eae7ffd264b0f603ed6a0
...
`prisma migrate dev --name init`を複数回実行すると、スキーマ全体を再作成するか聞いてくることがあります。yesを選ぶと、パブリック・スキーマが削除され、再作成されます。この後、デフォルトの許可（grant）がなくなります。この問題に遭遇した場合は、許可（grant）を修正するためのヘルパーSQLを追加してください。
