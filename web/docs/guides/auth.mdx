...
「Authentication」→「Settings」→「External OAuth Providers」と進みます。それぞれに「Client ID」と「Secret」を入力することで、プロバイダーを有効できます。
...
#### プライベートテーブルのリアルタイム機能を無効化

当社のリアルタイムサーバーは、ユーザーごとのセキュリティーを提供していません。WebSocket用のより強固な認証システムを構築するまでは、プライベートテーブルのリアルタイム機能を無効できます。これを行うには、Postgresのレプリケーションのパブリケーションの管理で行います。
...
現在、[リアルタイムのセキュリティーの拡張](https://github.com/supabase/walrus)の開発を進めています。
