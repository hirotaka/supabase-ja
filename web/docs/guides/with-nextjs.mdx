...
これらの変数はブラウザー上で公開されますが、データベースに[行レベルセキュリティー](/docs/guides/auth#row-level-security)が設定されているので、全く問題ありません。
...
また、オプションとして、[こちら](https://raw.githubusercontent.com/supabase/supabase/master/examples/react-user-management/src/index.css)のCSSファイルで`styles/globals.css`を更新して、アプリの外観を整えます。
...
ユーザーがプロフィール写真をアップロードできるように、アバターを作成してみましょう。
